...
Per event (from Step 1) we add:
  ...
  P_eff_mm          exponentially-weighted cumulative rainfall through event
                    start (k=0.9, 30-day): P_eff = P_t + k*P_t-1 + k^2*P_t-2 + ...
...
def api_series(daily):
    """Effective rainfall: P_eff(t) = sum_{n=0..N} k^n * P(t-n)
    (includes day t's own rainfall, weight k^0=1, per exponentially-weighted
    cumulative rainfall definition)."""
